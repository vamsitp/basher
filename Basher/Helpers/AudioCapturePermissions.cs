    ...
    public class AudioCapturePermissions
    {
        ...
        public static async Task<bool> RequestMicrophonePermission()
        {
            ...
            catch (TypeLoadException tle)
            {
                ...
                var messageDialog = ServiceLocator.Current.GetInstance<IDialogService>();// new Windows.UI.Popups.MessageDialog("No Audio Capture devices are present on this system.");
                await messageDialog.ShowError(tle, "Media player components are unavailable.", "OK", null);
                result = false;
            }
            ...
            catch (Exception exception)
            {
                ...
                {
                    var messageDialog = ServiceLocator.Current.GetInstance<IDialogService>();// new Windows.UI.Popups.MessageDialog("No Audio Capture devices are present on this system.");
                    await messageDialog.ShowError(exception, "No Audio Capture devices are present on this system.", "OK", null);
                    result = false;
                }
                else
                {
